...
CM172YSF::CM172YSF(const std::string &configFile) : m_callsign(),
                                                    m_m17Ref(),
                                                    m_conf(configFile),
                                                    m_conv(),
                                                    m_m17Frame(NULL),
...
int CM172YSF::run()
{
    ...
    std::string ysf_strAddress = m_conf.getYSFDstAddress();
    in_addr ysf_dstAddress = CUDPSocket::lookup(ysf_strAddress);
    ...
    LogMessage("YSF Ref %s Port %d\n", ysf_strAddress.c_str(), ysf_dstPort);
    ...
    if (m_ysfNetwork->writePoll())
    {
        LogMessage("YSF Write Poll True\n");
    }
    else
    {
        LogMessage("YSF Write Poll False\n");
    }
    ...
    for (; end == 0;)
    {
        ...
        while (m_m17Network->readData(m_m17Frame, 54U) > 0U)
        {
            ...
            if (!memcmp(m_m17Frame, "M17 ", 4))
            {
                ...
                ysf_src = css;
                memcpy(cs, m_m17Frame + 4, 6);
                decode_callsign(cs);
                std::string cssd((char *)cs);
                cssd = cssd.substr(0, cssd.find(' '));
                ysf_dst = cssd;
                m_m17Frames++;
            }
        }
                    ...
                    if (dt == YSF_DT_VD_MODE2)
                    {
                        ...
                        if (fi == YSF_FI_HEADER)
                        {
                            if (ysfPayload.processHeaderData(buffer + 35U))
                            {
                                ...
                                memset(m17_src, ' ', 9);
                                std::string css = ysfSrc;
                                memcpy(m17_src, css.c_str(), 8);
                                m17_src[8] = 'Y';
                                m17_src[9] = 0x00;
                                encode_callsign(m17_src);
                                m_conv.putYSFHeader();
        ...
        if (ysfWatch.elapsed() > YSF_FRAME_PER)
        {
            ...
            if (ysfFrameType == TAG_HEADER)
            {
                ...
                ::memcpy(m_ysfFrame + 14U, ysf_src.c_str(), YSF_CALLSIGN_LENGTH);
                ...
                memcpy(csd1 + YSF_CALLSIGN_LENGTH, ysf_src.c_str(), YSF_CALLSIGN_LENGTH);
            ...
            else if (ysfFrameType == TAG_DATA)
            {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    ysfPayload.writeVDMode2Data(m_ysfFrame + 35U, (unsigned char *)ysf_src.c_str());
                    break;
                case 2:
                    ysfPayload.writeVDMode2Data(m_ysfFrame + 35U, (unsigned char *)ysf_dst.c_str());
                    break;
                case 5:
                    memset(dch, ' ', YSF_CALLSIGN_LENGTH / 2);
                    memcpy(dch + YSF_CALLSIGN_LENGTH / 2, m_conf.getYsfRadioID().c_str(), YSF_CALLSIGN_LENGTH / 2);
